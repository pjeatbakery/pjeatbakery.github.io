...
function TwojSlideshowDuplicate_ListingLink($actions, $post){
	if( TwojSlideshowDuplicate_slideshowOnly( $post->post_type ) ){
		$actions['clone'] 				= '<a href="'.TwojSlideshowDuplicate_getUrl( $post->ID , 'display', false)	.'" title="'.esc_attr(__("Clone this item", 'twoj_slideshow')). '">'		.__('Clone', 'twoj_slideshow')		.'</a>';
		$actions['edit_as_new_draft'] 	= '<a href="'.TwojSlideshowDuplicate_getUrl( $post->ID )						.'" title="'.esc_attr(__('Copy to a new draft', 'twoj_slideshow')).'">'	.__('New Draft', 'twoj_slideshow')	.'</a>';
	}
	return $actions;
}
